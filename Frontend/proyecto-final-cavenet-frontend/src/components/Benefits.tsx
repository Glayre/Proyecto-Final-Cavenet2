    ...
    <section className="px-6 py-12 bg-background">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
        {items.map((item, i) => (
          <div
            key={i}
            className="bg-cavGray rounded-xl p-6 text-center shadow-cav hover:scale-105 transition-transform"
          >
            <div className="text-cavenetIndigo mb-4 flex justify-center">{item.icon}</div>
            <h3 className="text-xl font-semibold mb-2 text-cavDark">{item.title}</h3>
